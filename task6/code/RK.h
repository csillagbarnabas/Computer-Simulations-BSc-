...
template<typename T, typename F,typename RHS>
void Euler_fo(std::vector<T>& x,const F dt, RHS f) {
    std::vector<T> d = f(x);
//    x[2] = x[2] + f(x)[2]*dt;
//    x[1] = x[1] + f(x)[1]*dt; //sajnos csak így működik
//    x[0] = x[0] + dt;
...
template<typename T, typename F,typename RHS1,typename RHS2>
void Euler_fo_2csat(std::vector<T>& x1,std::vector<T>& x2,const F dt, RHS1 f1,RHS2 f2){
    std::vector<T> d1 = f1(x1,x2);
    std::vector<T> d2 = f2(x1,x2);
//    x[2] = x[2] + f(x)[2]*dt;
//    x[1] = x[1] + f(x)[1]*dt; //sajnos csak így működik
//    x[0] = x[0] + dt;
     x1 = {x1[0] + dt,x1[1] + dt*d1[1]};
     x2 = {x2[0] + dt,x2[1] + dt*d2[1]};
}

template<typename T, typename F, typename RHS1, typename RHS2>
void RK4Step_2(std::vector<T>& x1,std::vector<T>& x2, F tau, RHS1 derivs1, RHS2 derivs2)
{
//    std::vector<T> k1 = tau * derivs(x);
    std::vector<T> perm = derivs1(x1,x2);
    std::vector<T> k1 = multiply_vs(derivs1(x1,x2),tau);
    std::vector<T> k2 = multiply_vs(derivs1(add(x1,multiply_vs(k1,0.5)),x2),tau);
    std::vector<T> k3 = multiply_vs(derivs1(add(x1,multiply_vs(k2,0.5)),x2),tau);
    std::vector<T> k4 = multiply_vs(derivs1(add(x1,k3),x2),tau);

    std::vector<T> perm2 = derivs2(x1,x2);
    std::vector<T> k21 = multiply_vs(derivs2(x1,x2),tau);
    std::vector<T> k22 = multiply_vs(derivs2(x1,add(x2,multiply_vs(k21,0.5))),tau);
    std::vector<T> k23 = multiply_vs(derivs2(x1,add(x2,multiply_vs(k22,0.5))),tau);
    std::vector<T> k24 = multiply_vs(derivs2(x1,add(x2,k23)),tau);

    x1 = add(x1,multiply_vs(add(add(k1, multiply_vs(k2,2)), add(multiply_vs(k3,2), k4)),1.0/6.0));
    x2 = add(x2,multiply_vs(add(add(k21, multiply_vs(k22,2)), add(multiply_vs(k23,2), k24)),1.0/6.0));
//    x = {x[0] + (k1[0]+k2[0]*2+k3[0]*2+k4[0])/6,x[1] + (k1[1]+k2[1]*2+k3[1]*2+k4[1])/6,x[2] + (k1[2]+k2[2]*2+k3[2]*2+k4[2])/6};
}