    ...
    for (int i = 0; i < N; i++){
        for (int j = 0; j < N; j++){
            if (i != j){
                ...
                sumpot -= 4*(1/pow(rij,6) - 1/pow(rij,3));
            }
...
double halfp(){
    ...
    for(int j = 0; j < N; j++){
        for(int i = 0; i < j; i ++){
            ...
            sum += 24*(2/pow(rij,6) - 1/pow(rij,3));
        }
    }
    return sum/3;
...
int main() {
    ...
    computeAccelerations();
    double dt = 0.01;
    int K = 5000; //number of steps 
    std::vector<double> Tv(K);
    std::vector<double> E(K);
    std::vector<double> psum(K);
    std::ofstream file("TE30m.data");
    for (int i = 0; i < K; i++) {
        velocityVerlet(dt);
        Tv[i] = instantaneousTemperature();
        E[i] = energy();
        psum[i] = halfp();
        file << Tv[i] << " " << E[i] << '\n';
        if (i % 200 == 0)
    ...
    double E1 = 0;
    double E2 = 0;
    double Teq = 0; //equelibrial temperature
    double P = 0;
    for (int i = 2000; i < K; i++){
        E2 += E[i]*E[i];
        E1 += E[i];
        Teq += Tv[i];
        P += psum[i];
    }
    E1 = (E1/(K-2000))*(E1/(K-2000));
    E2 = (E2/(K-2000));
    Teq = (Teq/(K-2000));
    P = ((P/(K-2000)) + N*Teq)/(L*L*L);
    double Z = P*(L*L*L)/(N*Teq);
    std::cout << "C_v = " << (E2-E1)/(Teq*Teq) << std::endl;
